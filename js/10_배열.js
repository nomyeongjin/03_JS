function check1(){
    ...
    */
    ...
    arr2[1]="눈온다";
    arr2[2]=true;
    // JS 배열의 특징 : 인덱스별로 자료형을 다르게 할 수 있다. (권장사항 x)
...
function selectMenu(){
    ...
    // 점심 메뉴로 초기화된 배열 생성
    const menus = ["김밥","제육볶음","돈까스","파스타",
    "샐러드","라면","햄버거","순대국밥","닭갈비","초밥"]

    // menus 배열 index범위 내에서 난수 생성
...
/* 고른 메뉴, 개수, 합계 금액 출력하기 */
// 버튼 클릭 시 prompt를 이용해서 메뉴명, 수량을 입력 받기
...
const menu =["라면","김밥","샌드위치","우동"];
const prices = [4000,3000,5000,6000];

// 주문한 메뉴 카운트 배열
const counts = new Array(menu.length); //-> 모든 칸이 비어있음

// 비어있는 칸을 0으로 채움
// for(let i=0; i<counts.length; i++){
//     counts[i]=0;
// }
counts.fill(0); // 비어있는 값 모두 0으로 채움
...
while(true){ //무한 반복
    ...
    //     // menu[i] 값이 같다면 idx에 i값을 저장
    //     if(selectMenu ==menu[i]){
    //         idx=i;
...
/* 2부터 2씩 증가하는 수를
   5행 5열배열에 모든 요소에 차례대로 대입하고 출력하기 
*/
function check6(){
    ...
    for(let row=0 ; row<5 ; row++){ // 행
        
        // 1차원 배열을 만들어 arr[i]에 대입
        arr[row]=[];

        for(let col=0; col<5; col++){ // 열
