...
function classTest2(){
  ...
  // -> HTML Collection(유사 배열) 형태로 얻어옴
    ...
    /* 
        (중요) 요소들을 한 번에 얻어오게 되면 배열 형태로 반환된다
...
function nameTest(){
    ...
    //name 속성값이 "hobby"인 요소를 모두 얻어와 hobbyList에 저장
    ...
    for(let i=0; i<hobbyList.length; i++){//순차접근
        ...
        // input요소.checked = falses -> 해당 요소를 체크 해제

        // 체크박스의 값, 체크 여부 출력
        console.log(hobbyList[i].value, hobbyList[i].checked);

        // 체크여부를 검사해서 체크가 되어 있다면 
...
function cssTest(){
    ...
    */
    ...
    // 요소의 테두리를 변경
    container.style.border = "10px solid #778da9";

    // 첫 번째 자식 div를 선택하는 방법 1
    // const div1 = document.querySelector('[target-div="css-div"] > div:first-child');
    
    // 첫 번째 자식 div를 선택하는 방법 2
